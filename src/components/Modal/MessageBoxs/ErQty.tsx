...
const ErQty: React.FC = (props) => {
  const { options, message, hide } = useMessageBoxService();
  const [qty, setQty] = useState(1);
  const maxQuantity =
    options?.customParameters?.maxQuantity !== undefined
      ? Number(options?.customParameters?.maxQuantity)
      : 1;
  const changeQuantity = (type: "plus" | "minus") => {
    if (type === "minus" && qty > 1) setQty(qty - 1);
    else if (type === "plus" && qty < maxQuantity) setQty(qty + 1);
  };
  return (
    <View style={styles.container}>
      <View style={styles.card}>
        ...
        <AppText
          selectable
          style={[
            { textAlign: "center", fontFamily: "Poppins_300Light_Italic" },
          ]}
        >
          {message}
        </AppText>
        ...
        >
          ...
          >
            <AppText labelType={LabelType.Label}>{qty}</AppText>
          </View>
          <AppButton
            style={styles.qtyButton}
            onPress={() => changeQuantity("plus")}
            transparent
          >
            <AppText>+</AppText>
          </AppButton>
