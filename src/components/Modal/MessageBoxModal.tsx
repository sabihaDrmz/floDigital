...
import FloTextBox from "../FloTextBox";
...
class MessageBoxModal extends Component {
  ...
  render() {
    ...
    //     }}>
    //     {MessageBox.options &&
    //       MessageBox.options.type !== undefined &&
    //       MessageBox.options.type === MessageBoxType.Standart && (
    //         <MbStandart
    ...
    //     {MessageBox.options &&
    //       MessageBox.options.type !== undefined &&
    //       MessageBox.options.type === MessageBoxType.YesNo && <View></View>}
    //     {/* Eğer messagebox ayarı yapılmadıysa */}
    //     {(MessageBox.options === undefined ||
    //       MessageBox.options.type === undefined) && (
    //       <MbStandart
    //         options={MessageBox.options}
    //         message={MessageBox.message}
    ...
    if (MessageBox.isShow) {
      if (MessageBox.buttonType === MessageBoxType.BasketNumber) {
        return (
          <View style={styles.container}>
            <View style={styles.card}>
              ...
              >
                {translate("messageBox.customerBasketNo")}
              </Text>
              <Text
                style={{
                  textAlign: "center",
              ...
              >
                {BasketService.selectedBasket.basketItems.filter(
                  (x) => !x.isOmc
                ).length > 0 && (
                  <AppText
                    style={{
      ...
      } else if (MessageBox.buttonType === MessageBoxType.SmsValidation) {
        return (
          <View style={styles.container}>
            <View style={styles.card}>
              <FloTextBox
                ...
                onlyNumber
                onChangeText={(input) =>
                  this.setState({ validateSmsCode: input })
                }
              />
              {MessageBox.message ? (
                <Text
              ...
              {MessageBox.timeLeft > 0 ? (
                ...
                >
                  {`${new Date(MessageBox.timeLeft * 1000)
                    .toISOString()
                    .substr(14, 5)}`}
                </Text>
              ) : (
                  ...
                  onPress={() => {
                    if (MessageBox.onResend) {
                      ...
                      MessageBox.Show(
                        ...
                        MessageBox.onComplete,
                        MessageBox.onCancelled
                      );
                      BasketService.sentKvkk();
                    }
                  }}
                />
              )}
              <FloButton
                ...
                onPress={() => {
                  if (MessageBox.onValidate) {
                    MessageBox.onValidate(this.state.validateSmsCode);
                  } else BasketService.validateKvkk(this.state.validateSmsCode);
                }}
              />
              <FloButton
      ...
      } else if (MessageBox.buttonType === MessageBoxType.StockOutValidation) {
        return (
          <View style={styles.container}>
            <View
              style={[
                styles.card,
                { paddingVertical: 35, paddingHorizontal: 50 },
              ]}
            >
              <Text
                style={{
              ...
              <FloButton
                onPress={() => {
                  if (MessageBox.onCancelled) MessageBox.onCancelled();
                  MessageBox.Hide();
                }}
                title={translate("messageBox.cancel")}
                containerStyle={{
      ...
      } else if (MessageBox.buttonType === MessageBoxType.OmsComplete) {
        return (
          <View style={styles.container}>
            <View style={styles.card}>
              <View
              ...
              {/* <View
                ...
                <Image
                  source={{uri: MessageBox.message.split('|')[1]}}
      ...
      } else if (MessageBox.buttonType === MessageBoxType.OrderNotFound) {
        return (
          <View style={styles.container}>
            <View style={styles.card}>
              <View
      ...
      } else if (MessageBox.buttonType === MessageBoxType.PrinterType){
        <View style={styles.container}>
            <View style={styles.card}>
              <View
                style={{
      ...
      return (
        <View style={styles.container}>
          {MessageBox.buttonType === 1 ? (
            <MbYesNo
              ...
              options={{
                noButtonEvent: () => {
                  MessageBox.onCancelled ? MessageBox.onCancelled() : undefined;
                  MessageBox.Hide();
                },
                yesButtonEvent: () => {
                  MessageBox.onComplete ? MessageBox.onComplete() : undefined;
                ...
                yesButtonTitle: MessageBox.yesButton,
                noButtonTitle: MessageBox.noButton,
              }}
            />
          ) : (
            <MbStandart
              message={MessageBox.message}
              options={{
                yesButtonTitle: translate("messageBox.ok"),
                yesButtonEvent: () => {
                  MessageBox.onComplete ? MessageBox.onComplete() : undefined;
                  MessageBox.Hide();
                },
        ...
        //   <MbStandart message={MessageBox.message} />
        //   <View style={styles.card}>
        //     <Text style={styles.description}>{MessageBox.message}</Text>
        //     <View
        //       style={{
        //         flexDirection: 'row',
        //         justifyContent:
        //           MessageBox.buttonType === 1 ? 'space-between' : 'flex-end',
        //         marginTop: 20,
        //       }}>
        //       {MessageBox.buttonType === 1 ? (
        //         <React.Fragment>
        //           <FloButton
        //             title={MessageBox.noButton}
        //             containerStyle={{
...
const OmsCompleteIcon: React.FC = (props) => {
  return (
    <Svg width={39.104} height={48} {...props}>
      <Defs></Defs>
      <G
        id="Group_3037"
