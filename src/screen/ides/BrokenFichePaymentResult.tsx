...
const BrokenFichePaymentResult: React.FC = () => {
  ...
  const { transaction, erCurrentFiche, selectedPaymentTypes, printDoc } = useEasyReturnService();

  if (
    (transaction?.easyReturnTrasactionLine === undefined ||
      transaction?.easyReturnTrasactionLine === null) &&
    transaction?.easyReturnTrasactionLines !== undefined
  )
    transaction.easyReturnTrasactionLine =
      transaction.easyReturnTrasactionLines;
  let selectedBarcodes =
    transaction?.easyReturnTrasactionLine?.map(
      (x) => x.barcode
    );

  if (selectedBarcodes === null || selectedBarcodes === undefined)
    selectedBarcodes = [];
  const totalPrice = erCurrentFiche?.data
