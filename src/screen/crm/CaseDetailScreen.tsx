    ...
    for (let index = 0; index < medias.length; index++) {
      ...
      if (Platform.OS !== "web") {
        if (medias[index].MediaType !== "video") {
          const fileInfo = await FileSystem.getInfoAsync(medias[index].Url);
          //@ts-ignore
          const fileSizeInMB = fileInfo.size / (1024 * 1024);

          if (Platform.OS === "ios") {
            // 1 ile 0.5 mb arasında ise ona göre küçültme
            ...
            // 1 ile 2 mb arasında ise ona göre küçültme
            ...
            // 0.5' den de küçükse hiç küçültme uygulanmıyor
            else {
              tmpMedias.push(medias[index]);
            }
            ...
            if (fileSizeInMB > 0.5) {
              const manipResult = await ImageManipulator.manipulateAsync(
                ...
                [
                  {
                    resize: {
                      width: medias[index].Width / 3,
                      height: medias[index].Height / 3,
                    },
                  },
                ],
                { compress: 0.2, format: ImageManipulator.SaveFormat.JPEG }
