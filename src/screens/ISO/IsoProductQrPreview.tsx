...
const IsoProductQrPreview: React.FC<any> = (props) => {
  const product = ProductService.product?.product;
  ...
  const [residualSearch, setResidualSearch] = useState(product!.barcode);
    ...
    <View style={styles.container}>
      ...
      <View style={{ flex: 1 }}>
        {product && (
          ...
          >
            {product.images && product.images.indexOf("flo-logo.svg") ? (
              <Image
              ...
              <Image
                source={ToImageCdnUri(product.images[0], 100, 100)}
                style={{
                  width: 100,
                  height: 100,
                  borderRadius: 14,
              ...
              <View style={{ flexDirection: "row" }}>
                ...
                <View style={{ justifyContent: "center" }}>
                  ...
                  <View style={{ flexDirection: "row", alignItems: "center" }}>
                    ...
                    >
                      Осталось {ProductService.kzQrCode?.quantity} штук QR
                    </AppText>
                  </View>
                </View>
              </View>
        ...
        >
          <AppButton
            ...
            onPress={() => {
              let printer = PrinterConfigService.selectedPrinter;
              let printerConfig = PrinterConfigService.printerConfig;
              if (
                printer === undefined ||
                ...
                MessageBoxNew.show(
                  "Пожалуйста, выберите конфигурацию этикетки",
                  {
              ...
              if (selectedRadio === 1) {
                ProductService.approveQrCode().then((res) => {
                  if (res) {
                    PrinterConfigService.printKzkQrCode();
                    closePopup();
                  }
                });
              } else if (
                  ...
                  >
                    <TouchableOpacity
                      ...
                      onPress={() => {
                        ...
                        setResidualSearch(product!.barcode);
                      }}
                    >
                      <AppText
                        style={{
                    ...
                    <AppButton
                      ...
                      onPress={() => {
                        if (!searchId || searchId === "") {
                          MessageBoxNew.show("Пожалуйста, введите QR-ID");
                          return;
                        }

                        ProductService.findQrCode(parseInt(searchId)).then(
                          (res) => {
                            setOpenQrPopup(false);
                            res ? setQrResult(true) : setQrResult(false);
                          }
                ...
                <View>
                  ...
                  >
                    <AppButton
                      ...
                      style={{ flex: 1, marginRight: 3 }}
                      onPress={() => {
                        ProductService.getQrCode(false, false, "").then(
                          (res) => {
                            setOpenQrPopup(false);
                            res ? setQrResult(true) : setQrResult(false);
                          }
                      ...
                      onPress={() => {
                        Actions["isoKzCamera"]({
                          ...
                          onReadBarcode: (barcode: string) => {
                            if (Actions.currentScene === "isoKzCamera")
                              Actions.pop();

                            ProductService.findKzQrCodeWithQrCode(barcode).then(
                              (res) => {
                                setOpenQrPopup(false);
                                res ? setQrResult(true) : setQrResult(false);
                              }
...
export default IsoProductQrPreview;
