...
const MAX_WIDTH = (width / 4) * 3 - 50;
const MIN_WIDTH = (width / 4) * 1 - 20;
const AnimatedTouchable = Animated.createAnimatedComponent(TouchableOpacity);
const CancellationScreen: React.FC = (props) => {
  const [eventType, setEventType] = useState<1 | 2>(1);
  const selectedType = useSharedValue(eventType === 1 ? 0 : 1);
  const animatedCargoStyle = useAnimatedStyle(() => {
    return {
      width: interpolate(
        selectedType.value,
        [0, 1],
  ...
  const animatedCargoInfoStyle = useAnimatedStyle(() => {
    return {
      width: interpolate(selectedType.value, [0, 1], [MAX_WIDTH - 100, 0.01]),
      opacity: interpolate(selectedType.value, [0, 1], [1, 0]),
    };
  });

  const animatedCargoInfoStyle2 = useAnimatedStyle(() => {
    return {
  ...
  const animatedStoreInfoStyle = useAnimatedStyle(() => {
    return {
      width: interpolate(selectedType.value, [0, 1], [0.01, MAX_WIDTH - 100]),
      opacity: interpolate(selectedType.value, [0, 1], [0, 1]),
    };
  });

  const animatedStoreInfoStyle2 = useAnimatedStyle(() => {
    return {
  ...
  const animatedStoreStyle = useAnimatedStyle(() => {
    return {
      width: interpolate(
        selectedType.value,
        [0, 1],
      ...
      // opacity: interpolate(selectedType.value, [0, 1], [0.5, 1]),
      backgroundColor: interpolateColor(
        selectedType.value,
        [0, 1],
    ...
    <>
                    ...
                    onPress={() => {
                      Actions["easyReturnCamera"]({
                        onReadComplete: (barcode: string) => {
                          if (Actions.currentScene === "isoCamera")
                            Actions.pop();
                          ...
                          if (eventType === 1)
                            ...
                            EasyReturnService.SearchFiche({
                              orderNo: barcode.trim().startsWith("9")
                                ? barcode
                                : "",
                              voucherNo: "",
                              ficheNumber: barcode.trim().startsWith("9")
                                ? ""
                                : barcode,
                            });
      ...
      <Observer>
        {() => {
          return (
            <>
              {EasyReturnService.CancellationOrderNotFoundPopup && (
                <BlurView
                  style={{
                    position: "absolute",
                    width,
